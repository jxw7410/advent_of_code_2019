def diagonostic_test(input)
  i = 0
  while i < input.length
    instruction = extract_instruction(input[i])
    opcode = instruction[0..1]
    if opcode == "10" || opcode == "20"
      target_index = input[i + 3]
      val1 = instruction[2] == "0" ? input[input[i + 1]] : input[i + 1]
      val2 = instruction[3] == "0" ? input[input[i + 2]] : input[i + 2]
      input[target_index] = opcode == "10" ? val1 + val2 : val1 * val2
      i += 4
    elsif opcode == "30"
      p "Please enter an input: "
      target_index = input[i + 1]
      input[target_index] = gets.chomp.to_i
      i += 2
    elsif opcode == "40"
      p instruction[2] == "0" ? input[input[i + 1]] : input[i + 1]
      i += 2
    elsif opcode == "50" || opcode == "60"
      target_val = instruction[2] == "0" ? input[input[i + 1]] : input[i + 1]
      if (opcode == "50" && target_val != 0) || (opcode == "60" && target_val == 0)
        i = instruction[3] == "0" ? input[input[i + 2]] : input[i + 2]
      else
        i += 3
      end
    elsif opcode == "70" || opcode == "80"
      val1 = instruction[2] == "0" ? input[input[i + 1]] : input[i + 1]
      val2 = instruction[3] == "0" ? input[input[i + 2]] : input[i + 2]
      if (opcode == "70" && val1 < val2) || (opcode == "80" && val1 == val2)
        input[input[i + 3]] = 1
      else
        input[input[i + 3]] = 0
      end
...
def extract_instruction(val)
  instruction = val.to_s.reverse
